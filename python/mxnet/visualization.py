...
def plot_network_detail(symbol, title="plot", save_format='pdf', shape=None, node_attrs={},
                 hide_weights=True):
    """Creates a visualization (Graphviz digraph object) of the given computation graph.
    Graphviz must be installed for this function to work.
                ...
                if input_name not in hidden_nodes:
                    ...
                    if draw_shape:
                        ...
                        if node["op"] == 'Convolution':
                            ...
                            label = attr["label"].split()
                            input_label = np.array(label[0].split('x'), dtype=int)
                            output_label = np.array(label[1].split('x'), dtype=int)

                            kernel = np.array(eval(params['kernel']), dtype=int)

                            if "num_group" in params and int(params["num_group"]) > 1:
                                compute_cost = np.prod(kernel) * np.prod(output_label[1:])
                                pass
                            else:
                                compute_cost = np.prod(kernel) * input_label[-3] * np.prod(output_label[1:])
                            #print(compute_cost)
                            attr["label"] += '  ' + str(compute_cost)

                            total_cost += compute_cost
                            if compute_cost > 20000000:
                                print('{} {} {} {}'.format(
                                        compute_cost,
                                        input_label[-3], output_label[-3], 
                                        name))
