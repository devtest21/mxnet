...
template<typename DType>
inline DType GetBboxLoss(const DType *mask, const DType *target, 
                         const DType *preds){
  DType loss = 0.0;
  for (int i=0; i<4; ++i) {
    DType x = (*(target + i) - *(preds + i)) * *(mask + i);
    // smooth L1
    if (std::abs(x) < 1) 
      loss += 0.5 * x * x;
    else
...
template<typename DType>
inline void MultiBoxTargetOhemForward(const Tensor<cpu, 2, DType> &loc_target,
                           ...
                           const Tensor<cpu, 2, DType> &loc_preds,
  ...
  for (int nbatch = 0; nbatch < num_batches; ++nbatch) {
    ...
    if (num_valid_gt > 0) {
      ...
      for (int j = 0; j < num_anchors; ++j) { 
        ...
        int idx = static_cast<int>(*(p_cls_target + j));
        DType prob = std::exp(p_cls_preds[j + num_anchors * idx] - max_val) / sum;
        // loss should be -log(x), but value does not matter, skip log

       
        DType eps = 1e-12;
        DType loss = -std::log(std::max(prob, eps));
        int offset = j * 4;
  
        DType bbox_loss = GetBboxLoss(p_loc_mask + offset, 
            p_loc_target + offset, 
            p_loc_preds + offset);

        using namespace std;
        cout << loss << ' ' <<bbox_loss << endl;
        loss += bbox_loss;
      ...
      int OHEM_SIZE = 256;
      for (int i = OHEM_SIZE; i < num_anchors; ++i) {
...
namespace op {
template<>
Operator *CreateOp<cpu>(MultiBoxTargetOhemParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new MultiBoxTargetOhemOp<cpu, DType>(param);
  });
  return op;
}

Operator* MultiBoxTargetOhemProp::CreateOperatorEx(Context ctx, std::vector<TShape> *in_shape,
...
DMLC_REGISTER_PARAMETER(MultiBoxTargetOhemParam);
MXNET_REGISTER_OP_PROPERTY(_contrib_MultiBoxTargetOhem, MultiBoxTargetOhemProp)
...
.add_argument("loc_pred", "NDArray-or-Symbol", "Location predictions.")
.add_arguments(MultiBoxTargetOhemParam::__FIELDS__());
}  // namespace op
}  // namespace mxnet
