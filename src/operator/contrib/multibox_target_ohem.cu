 ...
 * \brief MultiBoxTargetOhem op
 * \author Joshua Zhang
*/
#include "./multibox_target_ohem-inl.h"
...
template<typename DType>
__global__ void HardExampleMining(const DType *overlaps, 
                               const DType *cls_preds, DType *cls_target,
                               DType *loc_preds, DType *loc_target, DType *loc_mask,
                               DType *anchor_flags, DType *buffer,
                               const float negative_mining_ratio,
                               const float negative_mining_thresh,
                               const int minimum_negative_samples,
  ...
  loc_preds += nbatch * num_anchors * 4;
  ...
  for (int i = threadIdx.x; i < num_anchors; i += num_threads) {
    ...
    if (anchor_flags[i] > -0.5) {
      ...
      int idx = static_cast<int>(cls_target[i]);
      DType prob = exp(cls_preds[i + num_anchors * idx] - max_val) / sum;
      
      DType eps = 1e-12;
      DType loss = - log(max(prob, eps));
      for (int j=0; j<4; ++j) {
        DType x = (loc_preds[i * 4 + j] - loc_target[i * 4 + j]) 
              * loc_mask[i * 4 + j];
        if (abs(x) < 1)
          loss += 0.5 * x * x;
        else
...
template<typename DType>
inline void MultiBoxTargetOhemForward(const Tensor<gpu, 2, DType> &loc_target,
                           ...
                           const Tensor<gpu, 2, DType> &loc_preds,
  ...
  cuda::CheckLaunchParam(init_block_dim, init_thread_dim, "MultiBoxTargetOhem Init");
  ...
  cuda::CheckLaunchParam(num_batches, num_threads, "MultiBoxTargetOhem Matching");
  ...
  */
    ...
    cuda::CheckLaunchParam(num_blocks, num_threads, "MultiBoxTargetOhem Negative");
 ...
 // }
  ...
  temp_space[4] = 0;
  DType *buffer = temp_space[4].dptr_;
  cuda::HardExampleMining<DType><<<num_batches, num_threads>>>(overlaps,
    cls_preds.dptr_, cls_target.dptr_,
    loc_preds.dptr_, loc_target.dptr_, loc_mask.dptr_,
    anchor_flags, buffer, negative_mining_ratio,
    negative_mining_thresh, minimum_negative_samples,
    num_anchors, num_labels, num_classes);
  MULTIBOX_TARGET_CUDA_CHECK(cudaPeekAtLastError());
  // end Gang Chen 
...
namespace op {
template<>
Operator *CreateOp<gpu>(MultiBoxTargetOhemParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new MultiBoxTargetOhemOp<gpu, DType>(param);
  });
  return op;
}
}  // namespace op
