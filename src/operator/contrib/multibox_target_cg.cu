...
template<typename DType>
inline void MultiBoxTargetCgForward(const Tensor<gpu, 2, DType> &loc_target,
  ...
  cuda::CheckLaunchParam(init_block_dim, init_thread_dim, "MultiBoxTargetCg Init");
  ...
  cuda::CheckLaunchParam(num_batches, num_threads, "MultiBoxTargetCg Matching");
  ...
  if (negative_mining_ratio > 0) {
    ...
    cuda::CheckLaunchParam(num_blocks, num_threads, "MultiBoxTargetCg Negative");
...
namespace op {
template<>
Operator *CreateOp<gpu>(MultiBoxTargetCgParam param, int dtype) {
  Operator *op = NULL;
  MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
    op = new MultiBoxTargetCgOp<gpu, DType>(param);
  });
  return op;
}
}  // namespace op
